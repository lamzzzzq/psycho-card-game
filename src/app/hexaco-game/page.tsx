...
import { useHexacoGameStore } from '@/stores/useHexacoGameStore';
import {
  useGameFeedback,
  FeedbackOverlays,
  useYourTurnNotifier,
...
import { DIMENSIONS, Dimension, GameCard } from '@/types/hexaco-game';
import { getTargetCounts } from '@/lib/hexaco-game/scoring';
import { getDeclaredDimensions, getRankings } from '@/lib/hexaco-game/game-logic';
...
import { PongPanel } from '@/components/hexaco-game/PongPanel';
...
export default function GamePage() {
  ...
  // HEXACO 遊戲用 HEXACO-60 測評的分數（useHexacoStore），不是大五測評。
  ...
  const dimName = useCallback((d: Dimension) => (locale === 'en' ? HEXACO_GAME_META[d].nameEn : HEXACO_GAME_META[d].name), [locale]);
  ...
  useEffect(() => {
    if (!game) {
      router.push(hexacoScores ? '/hexaco-lobby' : '/hexaco/assess');
    }
  }, [game, hexacoScores, router]);
  ...
  const handleDiscardCard = useCallback((cardId: number) => {
    const g = useHexacoGameStore.getState().game;
    if (!g) return;
  ...
  const targets = getTargetCounts(humanPlayer.hexacoScores);
        ...
        <FilingProgressCard
          ...
          info={
            isHumanTurn && isDiscarding && !viewMode && !pongIntent && discardPickId === null ? (
              ...
              <span className="ml-auto truncate font-medium">{tg.doneLabel} {humanPlayer.declaredSets.length}/6</span>
            )
          }
          targets={targets}
      ...
      {game.phase === 'game-over' && (
        <GameOverModal
          ...
          onPlayAgain={() => {
            ...
            setViewedCardIds([]);
            setViewUsedThisTurn(false);
            if (hexacoScores) initGame(hexacoScores, game.settings);
          }}
          onBackToLobby={() => {
            resetGame();
            router.push('/');
