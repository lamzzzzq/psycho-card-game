...
// SD4 教學頁（src/app/hexaco-tutorial/page.tsx 的四維物理隔離副本）。
// 差異：sd4 卡面/組件/文案詞典、沙盒聚焦維度 情緒性→自戀(N,目標4)、
// 四維目標板與四維菱形雷達、規則手冊/測評路由指向 sd4 版；
// 沙盒卡改用【真題題面】（SD4 卡圖 28 張全套，imageId=真題 id → 圖文一致）。

import { useEffect, useMemo, useReducer, useRef, useState, type ReactNode } from 'react';
import { useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';
import { Sd4Card } from '@/components/game/Sd4Card';
import { cardToSd4Props } from '@/components/sd4-game/cardToSd4Props';
import { SD4_GAME_META } from '@/data/sd4-game-dimensions';
import { DIMENSIONS } from '@/types/sd4-game';
import { useHydrated } from '@/stores/useHydration';
import { useLocaleStore, STRINGS, type Locale } from '@/lib/i18n';
import { SD4_TUTORIAL_T } from '@/lib/i18n/sd4-tutorial';
// 目標板直接用正式牌桌那塊（見沙盒裏的調用註釋）——同一塊 UI 不留兩份實現。
import { FilingProgressCard } from '@/components/sd4-game/FilingProgressCard';
import type { GameCard, PersonalityCard, DummyCard, Dimension } from '@/types/sd4-game';

type TutStrings = (typeof SD4_TUTORIAL_T)['zh'];
...
// 教学沙盒卡用自造 id(101+)。SD4 直接用真題題面 + imageId=該題 id（1-28）→ 圖文一致、不刷 404。
const PC = (id: number, dim: Dimension, text: string, textEn: string, img: number): PersonalityCard => ({
  id, dimension: dim, text, textEn, imageId: img,
});
...
// 沙盒目標板：刻意讓只有自戀(4)剛好夠碰，其餘維度手牌都不足 → 教學聚焦在自戀。
// S 有 2<3、P 有 2<3、M 有 1<2、N 有 4=4。
// （S=3 讓後面截胡碰 2+1=3 成立；P 對子做後備，同樣 2+1=3）
const SANDBOX_TARGETS: Record<Dimension, number> = { M: 2, N: 4, P: 3, S: 3 };
// 開場彈窗的目標板：還沒開局，一個都沒歸檔。模塊級常量，免得每次渲染新建 Set
// 讓 FilingProgressCard 白白重繪。
const EMPTY_DIMS: Set<Dimension> = new Set();

// 歸檔 4 張自戀後，手裏留兩對（虐待 107/108、病態人格 105/106）+ 一張單張 +
// 知識牌 → 棄 1 張後仍至少剩一對可用於「截胡碰」演示。
// 題面全部取真題（sd4-questions），imageId=該題 id → 卡圖與題面一致。
// 對子優先虐待——Sadism(6 字母)是四維裏英文名最短的，卡面角標最清晰（沿老板 0814 反饋的判據）。
const SANDBOX_HAND: GameCard[] = [
  PC(101, 'N', '人們視我為天生的領導者。', 'People see me as a natural leader.', 8),
  PC(102, 'N', '我具有說服他人的獨特天分。', 'I have a unique talent for persuading people.', 9),
  PC(103, 'N', '我擁有一些出類拔萃的特質。', 'I have some exceptional qualities', 12),
  PC(104, 'M', '掌握情勢是需要事先計畫的。', 'Manipulating the situation takes planning.', 5),
  PC(105, 'P', '我傾向先投入眼前的情況，晚一點再來問問題。', 'I tend to dive in, then ask questions later.', 18),
  PC(106, 'P', '我有時會將自己置身於危險的情況之中。', 'I sometimes get into dangerous situations.', 20),
  PC(107, 'S', '看別人打架會讓我感到興奮。', 'Watching a fist-fight excites me.', 22),
  PC(108, 'S', '我很享受觀看暴力運動。', 'I enjoy watching violent sports.', 25),
  DC(109, '特質理論', 'Trait Theory', '認為人格是由穩定且可測量的特質所組成。', 'Views personality as a configuration of stable, measurable traits.'),
];
const SANDBOX_DRAWN: PersonalityCard = PC(110, 'N', '我有可能是某個領域的明日之星。', 'I\'m likely to become a future star in some area.', 13);
// 「查看 2 張」教學指定可看的兩張：107 虐待 + 110 剛抽到的自戀。玩家手動點選它們揭開。
// 選 107 是因為 Sadism 是四維裏英文名最短的——卡面維度角標字號按名字長度縮檔，
// 長名（Machiavellianism）會縮到最小檔看不清（老板 0814 反饋）。
const VIEW_IDS = [107, 110];
// 對手棄牌（截胡碰用）：維度在 open-claim 時按手裏現存的對子動態選定。題面/插畫與手牌錯開。
const CLAIM_CARDS: Record<Dimension, PersonalityCard> = {
  M: PC(111, 'M', '奉承他人是個讓別人站在你這邊的好方法。', 'Flattery is a good way to get people on your side.', 6),
  N: PC(111, 'N', '我喜歡時不時地炫耀。', 'I like to show off every now and then.', 14),
  P: PC(111, 'P', '我傾向與權威人士以及他們制定的規範抗爭。', 'I tend to fight against authorities and their rules.', 16),
  S: PC(111, 'S', '看傻瓜跌得一敗塗地很有趣。', 'It\'s funny when idiots fall flat on their face.', 24),
};
// 食胡課專用：已歸檔 3 維（M/P/S），手裏剩 3 張自戀，對手打出第 4 張自戀 → 正好食胡。
const HU_HAND: GameCard[] = [
  PC(201, 'N', '人們視我為天生的領導者。', 'People see me as a natural leader.', 8),
  PC(202, 'N', '我具有說服他人的獨特天分。', 'I have a unique talent for persuading people.', 9),
  PC(203, 'N', '我擁有一些出類拔萃的特質。', 'I have some exceptional qualities', 12),
];
const HU_DISCARD: PersonalityCard = PC(204, 'N', '我有可能是某個領域的明日之星。', 'I\'m likely to become a future star in some area.', 13);
const HU_DECLARED: Dimension[] = ['M', 'P', 'S'];
...
function createReducer(s: TutStrings, dimName: DimName) {
  ...
  switch (action.type) {
    ...
    case 'open-pong':
      ...
      // 不再 reveal 全場：靠 spotlight + 旁白點名 4 張自戀。
    ...
    case 'open-claim': {
      ...
      // 正常劇本選虐待(107/108)；玩家若把 S 對子棄掉一張，退回病態人格(105/106)。
      const CLAIM_DIM_PRIORITY: Dimension[] = ['S', 'N', 'P', 'M'];
      const claimDim = (CLAIM_DIM_PRIORITY.find((d) => (counts[d] ?? 0) >= 2) ?? 'S') as Dimension;
    ...
    case 'enter-hu':
      ...
      // 切到食胡課的劇本：其餘三維已歸檔，手裏剩 3 張自戀，揭開讓玩家看清。
      return {
        ...state,
        scene: 'hu-window',
        hand: HU_HAND,
...
function InteractiveSandbox({
  ...
  const FOCUS = SD4_GAME_META.N; // 沙盒聚焦維度（四維版=自戀；六維版是情緒性）
  const archived = state.chosenDim ? SD4_GAME_META[state.chosenDim] : FOCUS;
  const claimed = state.claimDim ? SD4_GAME_META[state.claimDim] : SD4_GAME_META.S;
  ...
  // 食胡步用固定的 HU_DECLARED（那一步是擺好的殘局；食胡成功後補上 N → 4/4 全實心），
  // 其餘看兩個歸檔標誌。
  const sandboxDeclaredDims = useMemo(
    () =>
      new Set<Dimension>(
        op === 'hu'
          ? (state.scene === 'hu-success' ? [...HU_DECLARED, 'N' as Dimension] : HU_DECLARED)
          : [
        ...
        <div className="mx-auto flex max-w-[26rem] flex-wrap justify-center gap-2">
          {all.map((c) => {
            ...
            return (
              ...
              >
                <Sd4Card
                  {...cardToSd4Props(locCard(c, loc), loc)}
                  width={73}
      ...
      <div className="space-y-3 rounded-[1.4rem] border border-[rgba(154,116,72,0.18)] bg-[linear-gradient(180deg,var(--psy-card-content),#f8f1e4)] p-4">
          ...
          {op === 'hu' ? (
            <>
              ...
              {state.scene === 'hu-success' ? (
                // 食胡成功：N 從虛線「缺口」變成與其他維度一致的實心歸檔 chip
                <span className="flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold"
                  style={{ color: FOCUS.colorHex, backgroundColor: FOCUS.colorHex + '20', border: `1px solid ${FOCUS.colorHex}55` }}>
                  <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: FOCUS.colorHex }} />
                  {s.archiveSetSuffix(dimName('N'), SANDBOX_TARGETS.N)}
                </span>
              ) : (
                <span className="rounded-full border border-dashed px-2 py-0.5 text-[10px] font-semibold"
                  style={{ color: FOCUS.colorHex, borderColor: FOCUS.colorHex + '88' }}>
          ...
          <div aria-hidden className="hidden pointer-events-none col-start-1 row-start-1 space-y-3 sm:block sm:invisible">
            ...
            <div className="grid gap-3 rounded-xl border p-3 text-xs sm:grid-cols-[auto_1fr]">
              <div className="flex justify-center">
                <Sd4Card {...cardToSd4Props(locCard(HU_DISCARD, loc), loc)} revealedDimension="N" width={73} />
              </div>
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            >
              ...
              <div className="relative tut-spotlight rounded-[0.55rem]">
                <Sd4Card faceDown text="" width={112} />
        ...
        {state.scene === 'pong-picking' && (
          <motion.div
            ...
            className="flex flex-col items-center gap-2 rounded-xl border border-[rgba(200,155,93,0.28)] bg-[rgba(200,155,93,0.08)] px-3 py-2 text-xs text-[var(--psy-accent)]"
          >
            <span style={{ color: state.chosenDim ? SD4_GAME_META[state.chosenDim].colorHex : FOCUS.colorHex }}>
        ...
        {state.scene === 'claim-dimension' && state.claimDim && (
          ...
          >
            <div className="flex justify-center">
              <Sd4Card {...cardToSd4Props(locCard(CLAIM_CARDS[state.claimDim], loc), loc)} revealedDimension={state.claimDim} width={73} />
            </div>
          ...
          >
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="mt-1 leading-6">
                {s.claimCardBodyA}
                <b style={{ color: SD4_GAME_META[state.claimDim].colorHex }}>{s.claimCardBodyMid(dimName(state.claimDim))}</b>
        ...
        <FilingProgressCard
          ...
          info={
            <span className="ml-auto shrink-0 font-medium">
              {STRINGS[loc].game.doneLabel} {sandboxDeclaredDims.size}/4
            </span>
          }
          targets={SANDBOX_TARGETS}
...
function RuleDiagram({ index, s, dimName }: { index: number; s: TutStrings; dimName: DimName }) {
  const c = SD4_GAME_META.N.colorHex; // 示例維度用自戀 N 的主題色（琥珀，同 HEXACO 示例 X 的視覺）
  const danger = '#d66a4f'; // 罰停／亮牌的紅

  switch (index) {
    case 0: { // 你的目標：每維各需要 N 張（數字=要湊幾張）→ 全部湊齊食胡
      const sample: Record<Dimension, number> = { M: 3, N: 4, P: 2, S: 3 };
      return (
          ...
          <div className="flex flex-wrap justify-center gap-1">
            {DIMENSIONS.map((d) => {
              ...
              return (
                ...
                >
                  {/* 維度名全寫，不再只給 M/N/P/S 單字母（同事反饋看不懂）。
    ...
    case 4: // 罰停：宣告失敗 → 罰停 1 回合 + 亮牌
      return (
        <DiagramFrame caption={s.dgFrozenCaption}>
          <span className="text-xl">⛔</span>
          <div className="flex gap-1">
            <MiniCard label="N" color={danger} />
            <MiniCard label="S" color={danger} />
          </div>
        </DiagramFrame>
      );
    ...
    case 5: // 聯機與勝負：無人胡 → 比已歸檔維度數排名（排名文字已在卡片正文，框內只留示意）
      return (
        <DiagramFrame>
          ...
          <span className="flex gap-0.5"><DonePill label="M" /><DonePill label="N" /><DonePill label="P" /></span>
          <Sym>›</Sym>
          <span className="text-base">🥈</span>
          <span className="flex gap-0.5"><DonePill label="S" /><DonePill label="M" /></span>
        </DiagramFrame>
      );
...
function FlowScreenshot({ mode, index, s }: { mode: 'pvp' | 'solo'; index: number; s: TutStrings }) {
  ...
  const portrait = (
    <div className="space-y-2.5">
      ...
      {/* 迷你四維雷達（示意）：替代原橫條，更直觀——也與流程說明「四維雷達圖」一致。
          原本重複的「聯機/單機/重新測評」按鈕排已移除（正式頁底部才有）。*/}
      <div className="flex justify-center py-0.5">
        <svg viewBox="0 0 100 100" className="h-44 w-44" aria-hidden>
          {/* 四維菱形雷達：外圈 R=34、內圈 R=17，頂點自 12 點鐘起每 90°。 */}
          <polygon points="50,16 84,50 50,84 16,50" fill="none" stroke="rgba(154,116,72,0.22)" strokeWidth="1" />
          <polygon points="50,33 67,50 50,67 33,50" fill="none" stroke="rgba(154,116,72,0.16)" strokeWidth="0.8" />
          {([[50, 16], [84, 50], [50, 84], [16, 50]] as const).map(([x, y], i) => (
            <line key={i} x1="50" y1="50" x2={x} y2={y} stroke="rgba(154,116,72,0.16)" strokeWidth="0.7" />
          ))}
          <polygon points="50,22.8 72.1,50 50,75.5 31.3,50" fill="rgba(195,154,82,0.32)" stroke="#c39a52" strokeWidth="1.6" />
          {([['M', 50, 22.8], ['N', 72.1, 50], ['P', 50, 75.5], ['S', 31.3, 50]] as const).map(([d, x, y]) => (
            <circle key={d} cx={x} cy={y} r="2.1" fill={SD4_GAME_META[d as Dimension].colorHex} />
          ))}
        </svg>
      </div>
    </div>
...
export default function TutorialPage() {
  ...
  const s: TutStrings = SD4_TUTORIAL_T[loc] as unknown as TutStrings;
  const dimName: DimName = (dim) => (loc === 'en' ? SD4_GAME_META[dim].nameEn : SD4_GAME_META[dim].name);
