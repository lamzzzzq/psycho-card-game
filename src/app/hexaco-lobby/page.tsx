...
// HEXACO 單機大廳（src/app/lobby/page.tsx 的物理隔離副本）：
// 分數源 = HEXACO-60 測評（useHexacoStore），開局走 useHexacoGameStore → /hexaco-game。
// 佈局/文案（LOBBY_T 是模型中立的）與 Big Five 大廳一致。
import { AI_PERSONAS } from '@/data/ai-personas';
import { AIDifficulty, RevealDifficulty } from '@/types/hexaco-game';

export default function LobbyPage() {
  const router = useRouter();
  const hexacoScores = useHexacoStore((st) => st.scores);
  const { initGame } = useHexacoGameStore();
  ...
  // 修 bug：登出后 localStorage 仍存 hexacoScores，光靠下面的 !hexacoScores 拦不住，
  // 会让未登录用户直接进单机开局。
  useEffect(() => {
  ...
  const handleStart = () => {
    initGame(hexacoScores, { totalRounds, aiDifficulty: difficulty, revealDifficulty }, avatar);
    router.push('/hexaco-game');
  };
