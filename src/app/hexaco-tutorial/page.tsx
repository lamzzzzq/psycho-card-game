...
// HEXACO 教學頁（src/app/tutorial/page.tsx 的六維物理隔離副本，2026-08-06）。
// 差異：hexaco 卡面/組件/文案詞典、沙盒聚焦維度 神經質→情緒性(E,目標4)、
// 六維目標板與六邊形雷達、規則手冊/測評路由指向 hexaco 版。

import { useEffect, useMemo, useReducer, useRef, useState, type ReactNode } from 'react';
import { useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';
import { HexacoCard } from '@/components/game/HexacoCard';
import { cardToHexacoProps } from '@/components/hexaco-game/cardToHexacoProps';
import { HEXACO_GAME_META } from '@/data/hexaco-game-dimensions';
import { DIMENSIONS } from '@/types/hexaco-game';
import { useHydrated } from '@/stores/useHydration';
import { useLocaleStore, STRINGS, type Locale } from '@/lib/i18n';
import { HEXACO_TUTORIAL_T } from '@/lib/i18n/hexaco-tutorial';
// 目標板直接用正式牌桌那塊（見沙盒裏的調用註釋）——同一塊 UI 不留兩份實現。
import { FilingProgressCard } from '@/components/hexaco-game/FilingProgressCard';
import type { GameCard, PersonalityCard, DummyCard, Dimension } from '@/types/hexaco-game';

type TutStrings = (typeof HEXACO_TUTORIAL_T)['zh'];
...
const SANDBOX_HAND: GameCard[] = [
  ...
  PC(103, 'E', '遇到困難時我需要別人的安慰', 'I need comfort from others when things get hard', 17),
  PC(104, 'C', '我會認真完成我承諾過的事', 'I follow through on what I promise', 2),
...
const CLAIM_CARDS: Record<Dimension, PersonalityCard> = {
  ...
  E: PC(111, 'E', '看到別人難過，我也會跟着難過', 'When others are sad, I feel sad too', 29),
  X: PC(111, 'X', '我在人群裏很有活力', 'I feel energetic among people', 4),
  A: PC(111, 'A', '我會站在別人的角度想', 'I see things from others\' perspective', 15),
  C: PC(111, 'C', '我做事一向井井有條', 'I keep things well-organized', 14),
  O: PC(111, 'O', '我喜歡探索新奇的點子', 'I enjoy exploring novel ideas', 7),
};
// 食胡課專用：已歸檔 5 維（H/X/A/C/O），手裏剩 3 張情緒性，對手打出第 4 張情緒性 → 正好食胡。
const HU_HAND: GameCard[] = [
  PC(201, 'E', '我經常為一些小事感到憂慮', 'I often worry about little things', 5),
  PC(202, 'E', '我容易情緒起伏', 'My feelings rise and fall easily', 11),
  PC(203, 'E', '遇到困難時我需要別人的安慰', 'I need comfort from others when things get hard', 17),
];
const HU_DISCARD: PersonalityCard = PC(204, 'E', '看到別人難過，我也會跟着難過', 'When others are sad, I feel sad too', 23);
const HU_DECLARED: Dimension[] = ['H', 'X', 'A', 'C', 'O'];
  ...
  switch (action.type) {
    ...
    case 'enter-hu':
      ...
      return {
        ...
        feedback: { tone: 'info', text: s.fbEnterHu(dimName('E')) },
      };
    case 'commit-hu':
...
function InteractiveSandbox({
  ...
  const N = HEXACO_GAME_META.E; // 沙盒聚焦維度（六維版=情緒性；變量名沿用大五版減小 diff）
  const archived = state.chosenDim ? HEXACO_GAME_META[state.chosenDim] : N;
  const claimed = state.claimDim ? HEXACO_GAME_META[state.claimDim] : HEXACO_GAME_META.A;
  ...
  const captionByScene: Record<Scene, string> = {
    ...
    'pong-dimension': s.captionPongDimension(dimName('E')),
    'pong-picking':
    ...
    'hu-window': s.captionHuWindow(dimName('E')),
        ...
        <div className="mx-auto flex max-w-[26rem] flex-wrap justify-center gap-2">
          {all.map((c) => {
            ...
            return (
              ...
              >
                <HexacoCard
                  {...cardToHexacoProps(locCard(c, loc), loc)}
                  width={73}
      ...
      <div className="space-y-3 rounded-[1.4rem] border border-[rgba(154,116,72,0.18)] bg-[linear-gradient(180deg,var(--psy-card-content),#f8f1e4)] p-4">
          ...
          <div aria-hidden className="hidden pointer-events-none col-start-1 row-start-1 space-y-3 sm:block sm:invisible">
            ...
            <div className="grid gap-3 rounded-xl border p-3 text-xs sm:grid-cols-[auto_1fr]">
              <div className="flex justify-center">
                <HexacoCard {...cardToHexacoProps(locCard(HU_DISCARD, loc), loc)} revealedDimension="E" width={73} />
              </div>
              <div className="flex flex-col justify-center gap-2">
                <div>
                  <div className="psy-serif text-sm">{s.huWho}</div>
                  <div className="mt-1 leading-6">{s.huBody(dimName('E'))}</div>
                </div>
          ...
          <div className="flex items-center justify-center gap-6">
            ...
            >
              ...
              <div className="relative tut-spotlight rounded-[0.55rem]">
                <HexacoCard faceDown text="" width={112} />
        ...
        {state.scene === 'pong-picking' && (
          <motion.div
            ...
            className="flex flex-col items-center gap-2 rounded-xl border border-[rgba(200,155,93,0.28)] bg-[rgba(200,155,93,0.08)] px-3 py-2 text-xs text-[var(--psy-accent)]"
          >
            <span style={{ color: state.chosenDim ? HEXACO_GAME_META[state.chosenDim].colorHex : N.colorHex }}>
        ...
        {state.scene === 'claim-dimension' && state.claimDim && (
          ...
          >
            <div className="flex justify-center">
              <HexacoCard {...cardToHexacoProps(locCard(CLAIM_CARDS[state.claimDim], loc), loc)} revealedDimension={state.claimDim} width={73} />
            </div>
          ...
          >
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="mt-1 leading-6">
                {s.claimCardBodyA}
                <b style={{ color: HEXACO_GAME_META[state.claimDim].colorHex }}>{s.claimCardBodyMid(dimName(state.claimDim))}</b>
        ...
        <FilingProgressCard
          ...
          info={
            <span className="ml-auto shrink-0 font-medium">
              {STRINGS[loc].game.doneLabel} {sandboxDeclaredDims.size}/6
            </span>
          }
          targets={SANDBOX_TARGETS}
          declaredDims={sandboxDeclaredDims}
          highlightDim={op === 'claim' ? state.claimDim : 'E'}
          disabled
          onOpenArchive={() => {}}
        />
...
function RuleDiagram({ index, s, dimName }: { index: number; s: TutStrings; dimName: DimName }) {
  const c = HEXACO_GAME_META.X.colorHex; // 示例維度用外向性 X 的主題色
  const danger = '#d66a4f'; // 罰停／亮牌的紅

  switch (index) {
    case 0: { // 你的目標：每維各需要 N 張（數字=要湊幾張）→ 全部湊齊食胡
      const sample: Record<Dimension, number> = { H: 3, E: 4, X: 2, A: 5, C: 4, O: 3 };
      return (
    ...
    case 5: // 聯機與勝負：無人胡 → 比已歸檔維度數排名（排名文字已在卡片正文，框內只留示意）
      return (
        <DiagramFrame>
          ...
          <span className="flex gap-0.5"><DonePill label="H" /><DonePill label="E" /><DonePill label="X" /></span>
          <Sym>›</Sym>
          <span className="text-base">🥈</span>
    ...
    <div className="space-y-2.5">
      ...
      <div className="flex justify-center py-0.5">
        <svg viewBox="0 0 100 100" className="h-44 w-44" aria-hidden>
          {/* 六邊形雷達（六維）：外圈 R=34、內圈 R=17，頂點自 12 點鐘起每 60°。 */}
          <polygon points="50,16 79.4,33 79.4,67 50,84 20.6,67 20.6,33" fill="none" stroke="rgba(154,116,72,0.22)" strokeWidth="1" />
          <polygon points="50,33 64.7,41.5 64.7,58.5 50,67 35.3,58.5 35.3,41.5" fill="none" stroke="rgba(154,116,72,0.16)" strokeWidth="0.8" />
          {([[50, 16], [79.4, 33], [79.4, 67], [50, 84], [20.6, 67], [20.6, 33]] as const).map(([x, y], i) => (
            <line key={i} x1="50" y1="50" x2={x} y2={y} stroke="rgba(154,116,72,0.16)" strokeWidth="0.7" />
          ))}
          <polygon points="50,24 73.5,36.5 70,64 50,76 28.5,61 30.5,38.5" fill="rgba(195,154,82,0.32)" stroke="#c39a52" strokeWidth="1.6" />
          {([['H', 50, 24], ['E', 73.5, 36.5], ['X', 70, 64], ['A', 50, 76], ['C', 28.5, 61], ['O', 30.5, 38.5]] as const).map(([d, x, y]) => (
            <circle key={d} cx={x} cy={y} r="2.1" fill={HEXACO_GAME_META[d as Dimension].colorHex} />
          ))}
        </svg>
      </div>
    </div>
...
export default function TutorialPage() {
  ...
  const s: TutStrings = HEXACO_TUTORIAL_T[loc] as unknown as TutStrings;
  const dimName: DimName = (dim) => (loc === 'en' ? HEXACO_GAME_META[dim].nameEn : HEXACO_GAME_META[dim].name);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            {mode === 'sandbox' ? (
              ...
              <>
                <button
                  onClick={() => router.push('/hexaco-rules')}
                  className="psy-btn flex-1 px-3.5 py-2 text-xs sm:flex-none sm:text-sm"
                >
                  {s.rulesHardcopy}
                </button>
