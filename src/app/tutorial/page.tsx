...
const STEPS: Step[] = [
  ...
  {
    title: '牌桌',
    body: '4 人對局（單機 = 你 + 3 個 AI，聯機 = 4 個真實玩家）。牌堆由兩類牌組成：帶顏色標記的「人格描述牌」（屬於某一維度），以及無維度歸屬的「檔案註記 / 知識牌」（中立，可棄可觀察對手風格）。',
  ...
  {
    title: '碰（公開歸檔）',
    body: '把同維度的牌湊齊目標張數即可歸檔。兩種觸發方式：\n• 自摸碰（你自己回合內，每回合最多 1 次）→ 從你的「手牌 + 剛抽到的牌」裏挑選恰好「該維度目標張數」張同維度牌；\n• 截胡碰（claim 窗口內先點先得）→ 從手牌挑選「該維度目標張數 − 1」張同維度牌，加上那張棄牌正好湊齊；\n選錯維度、混入其他人格牌、張數不對 → 視爲「碰失敗」，喫罰停一整輪。',
    hint: '系統不會告訴你「這個維度你夠了」，5 個未歸檔維度全部讓你選，需要自己判斷。',
  },
  {
    title: '食胡（宣佈勝利）',
    body: '當你的牌（已歸檔 + 手中 + 這張待判定牌）正好完成全部 5 個維度時，按下「食胡」。\n觸發時機：\n• 你的回合中（自摸食胡）；\n• 任意人棄牌進入判讀窗口時（截胡食胡）。\n判定不成立 → 食胡失敗，整副手牌公開 + 罰停一整輪。',
  },
  {
    title: '罰停一整輪（失敗的懲罰）',
    body: '「碰失敗 / 自摸碰失敗 / 食胡失敗」都罰停一整輪：\n• 失敗時押的那幾張牌（碰）或整副手牌（食胡）立刻公開；\n• 下一個本應抽出的回合被完整跳過（不抽不出）；\n• 期間所有其他玩家棄牌的判讀窗口你都不能參與；\n• 自摸碰失敗更重 —— 因爲你已經看過剛抽的牌再做錯，期間錯過的 claim 窗口約 6 個 + 跳 1 次自己回合，要再下下個回合抽出才解鎖。\n所有玩家都會在你頭像下看到「⛔ 罰停一輪」標識。',
    hint: '罰停期間也不能宣告食胡 —— 別想着低成本試錯。',
  },
  {
  ...
  switch (action.type) {
    ...
    case 'commit-pong': {
      ...
      return {
        ...
        feedback: {
          tone: 'fail',
          text: `自摸碰失敗：${reason}。真實遊戲會罰停一整輪，並公開你剛剛押錯的牌。`,
        },
...
function InteractiveSandbox({ onClose }: { onClose: () => void }) {
  ...
  const captionByScene: Record<Scene, string> = {
    ...
    'pong-failed': '失敗會公開你押錯的牌並罰停一整輪。點「繼續」回到選牌模式。',
