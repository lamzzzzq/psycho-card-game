...
// SD4 單機大廳（src/app/hexaco-lobby/page.tsx 的物理隔離副本）：
// 分數源 = SD4 測評（useSd4Store），開局走 useSd4GameStore → /sd4-game。
// 佈局/文案（LOBBY_T 是模型中立的）與 Big Five 大廳一致。
import { AI_PERSONAS } from '@/data/ai-personas';
import { AIDifficulty, RevealDifficulty } from '@/types/sd4-game';

export default function LobbyPage() {
  const router = useRouter();
  const sd4Scores = useSd4Store((st) => st.scores);
  const { initGame } = useSd4GameStore();
  ...
  // 修 bug：登出后 localStorage 仍存 sd4Scores，光靠下面的 !sd4Scores 拦不住，
  // 会让未登录用户直接进单机开局。
  useEffect(() => {
  ...
  const handleStart = () => {
    initGame(sd4Scores, { totalRounds, aiDifficulty: difficulty, revealDifficulty }, avatar);
    router.push('/sd4-game');
  };
