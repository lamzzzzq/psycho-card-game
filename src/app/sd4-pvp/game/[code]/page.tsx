...
// SD4 聯機牌桌（src/app/hexaco-pvp/game/[code]/page.tsx 的四維物理隔離副本）。
// 差異：sd4 store/引擎/組件、卡面 Sd4Card + /cards/sd4/ 圖、4 維計數與網格、
...
import { SerializedPlayer, PvpAction } from '@/types/sd4-pvp';
import { GameCard, GameAction, Player, PlayerId, DeclaredSet, Dimension, DIMENSIONS, isPersonalityCard } from '@/types/sd4-game';
import {
  useGameFeedback,
  FeedbackOverlays,
  useYourTurnNotifier,
...
import { leaveRoom, leaveAllRooms, updateRoomStatus } from '@/lib/sd4-game/room-api';
import { retryPendingSaves, removePendingInterrupted } from '@/lib/sd4-game/game-record';
...
import { getDeclaredDimensions } from '@/lib/sd4-game/game-logic';
...
export default function PvpGamePage() {
  ...
  const dimName = (d: Dimension) => (locale === 'en' ? SD4_GAME_META[d].nameEn : SD4_GAME_META[d].name);

  const { player } = useSd4PlayerStore();
  const { gameState, myPlayerId, isHost, sendMessage, subscribeRoom, offlinePlayerIds, room, roomClosed } = useSd4PvpStore();
  ...
  useEffect(() => {
    ...
    const { channel } = useSd4PvpStore.getState();
  ...
  useEffect(() => {
    const buffer = () => {
      try { useSd4PvpStore.getState().bufferInterruptedSnapshot(); } catch {}
    };
  ...
  useEffect(() => {
    const onVis = () => {
      const st = useSd4PvpStore.getState();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data && gameDeck !== 'sd4') {
        router.replace(gameDeck === 'hexaco' ? `/hexaco-pvp/game/${code}` : `/pvp/game/${code}`);
        return;
      }
  ...
  // dest：離開後去哪。默認回主頁；終局結算頁的組員「返回大廳」傳 '/sd4-pvp'。
  async function handleAbandonRoom(dest: string = '/') {
    const { isHost, gameState: gs, room, sendMessage: send } = useSd4PvpStore.getState();
    ...
    if (isHost) {
      ...
      if (gameInProgress) {
        ...
        try { useSd4PvpStore.getState().persistInterruptedGame(); } catch {}
      }
  ...
  function dispatchAction(action: PvpAction) {
    if (isHost) {
      useSd4PvpStore.getState().handlePlayerAction(myId!, action);
    } else {
  ...
  //   組員：只有「返回大廳」一個鈕 → handleAbandonRoom('/sd4-pvp')（leaveRoom 讓出
  ...
  if (gameState.winner) {
    return (
      <GameOverModal
        ...
        onPlayAgain={isHost ? () => router.replace(`/sd4-pvp/room/${code}`) : undefined}
        onBackToLobby={() => handleAbandonRoom(isHost ? '/' : '/sd4-pvp')}
  ...
  const targets = mePlayer ? getTargetCounts(mePlayer.sd4Scores) : null;
            ...
            info={
              <>
                ...
                {isMyTurn && isDiscarding && !viewMode && !pongIntent && discardPickId === null ? (
                  ...
                  <span className="ml-auto shrink-0 font-medium">{t.doneLabel} {mePlayer.declaredSets.length}/4</span>
                )}
              </>
            }
            targets={targets ?? {}}
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="w-24 rounded-xl ring-1 ring-[rgba(200,155,93,0.35)]">
                    <Sd4Card
                      ...
                      imageSrc={isPersonalityCard(gameState.pendingDiscard) ? `/cards/sd4/${gameState.pendingDiscard.imageId ?? gameState.pendingDiscard.id}.webp` : undefined}
                      locale={locale}
                      fluid
                    />
                  </div>
          ...
          >
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {DIMENSIONS.map((d) => {
                  const score = mePlayer.sd4Scores[d];
