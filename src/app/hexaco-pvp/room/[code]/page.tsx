...
// HEXACO 聯機房間等待頁（src/app/pvp/room/[code]/page.tsx 的六維物理隔離副本）。
// 差異：hexaco store/api、分數字段 hexaco、牌組守衛（大五房 → 重定向 /pvp/room/...）。
...
import { getRoomPlayers, kickPlayer, dissolveRoom, leaveRoom, updateRoomStatus } from '@/lib/hexaco-game/room-api';
import { Room, RoomSettings } from '@/types/hexaco-pvp';
...
export default function RoomWaitPage() {
  ...
  useEffect(() => {
    ...
    async function loadRoom() {
      try {
        ...
        useHexacoPvpStore.setState({ gameState: null, rawGameState: null });
        ...
        // 自己这一行的 student_id/hexaco（恒为 null），自己又收不到自己的 broadcast
        // (self:false)。用本地已知信息补上自己这一行，否则 host 开局会用 null/随机分数
        // 建局并存档（污染数据）。其他玩家信息靠 broadcast(player-joined/hexaco-updated) 收齐。
        const patchedList = playerList.map((p) =>
          p.player_id === player!.id
            ? {
                ...p,
                student_id: player!.studentId,
                hexaco: player!.hexaco ?? hexacoScores ?? null,
        ...
        useHexacoPvpStore.setState({ isHost: amHost });
        ...
        if (!amHost) {
          setTimeout(() => {
            ...
            useHexacoPvpStore.getState().sendMessage({
              type: 'player-joined',
              player: {
                id: player!.id,
                studentId: player!.studentId,
                hexaco: player!.hexaco,
                avatar: player!.avatar,
              },
  ...
  useEffect(() => {
    if (!hexacoScores || !player) return;
    sendMessage({ type: 'hexaco-updated', playerId: player.id, hexaco: hexacoScores });
  }, [hexacoScores]);

  useEffect(() => {
    let prev = useHexacoPvpStore.getState().gameState;
    return useHexacoPvpStore.subscribe(state => {
    ...
    const sub = supabase
      ...
      .on('postgres_changes', {
        ...
        if (next === 'playing') {
          ...
          useHexacoPvpStore.setState({ roomClosed: true });
        }
      })
      .subscribe();
  ...
  if (error) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => router.replace('/hexaco-pvp')} className="psy-btn psy-btn-ghost px-6 py-2 text-sm">
          {t.backLobby}
        </button>
      </div>
    );
                    ...
                    <>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-[var(--psy-muted)]">
                          {p.hexaco ? t.assessedTrue : t.assessedFalse}
                        </div>
                      </div>
                      {isHost && !isMe && (
                        <button
