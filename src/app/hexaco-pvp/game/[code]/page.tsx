...
// HEXACO 聯機牌桌（src/app/pvp/game/[code]/page.tsx 的六維物理隔離副本）。
// 差異：hexaco store/引擎/組件、卡面 HexacoCard + /cards/hexaco/ 圖、6 維計數與網格、
// 玩法教學按鈕暫不掛（RULES_T 是大五定稿文案）。大五側修 bug 記得同步。

import { useEffect, useState, useCallback, useRef } from 'react';
import { motion } from 'framer-motion';
import { useParams, useRouter } from 'next/navigation';
import { useHexacoPlayerStore } from '@/stores/useHexacoPlayerStore';
import { useHexacoPvpStore } from '@/stores/useHexacoPvpStore';
import { SerializedPlayer, PvpAction } from '@/types/hexaco-pvp';
import { GameCard, GameAction, Player, PlayerId, DeclaredSet, Dimension, DIMENSIONS, isPersonalityCard } from '@/types/hexaco-game';
import {
  useGameFeedback,
  FeedbackOverlays,
  useYourTurnNotifier,
...
import { getTargetCounts } from '@/lib/hexaco-game/scoring';
import { getDeclaredDimensions } from '@/lib/hexaco-game/game-logic';
import { PlayerHand } from '@/components/hexaco-game/PlayerHand';
import { DiscardControls } from '@/components/game/DiscardControls';
import { OpponentHand } from '@/components/hexaco-game/OpponentHand';
import { DrawPile } from '@/components/game/DrawPile';
import { DiscardPile } from '@/components/hexaco-game/DiscardPile';
import { GameLog } from '@/components/hexaco-game/GameLog';
import { DeclaredArea } from '@/components/hexaco-game/DeclaredArea';
import { FilingProgressCard } from '@/components/hexaco-game/FilingProgressCard';
import { GameOverModal } from '@/components/hexaco-game/GameOverModal';
...
export default function PvpGamePage() {
  ...
  const dimName = (d: Dimension) => (locale === 'en' ? HEXACO_GAME_META[d].nameEn : HEXACO_GAME_META[d].name);

  const { player } = useHexacoPlayerStore();
  const { gameState, myPlayerId, isHost, sendMessage, subscribeRoom, offlinePlayerIds, room, roomClosed } = useHexacoPvpStore();
  ...
  useEffect(() => {
    if (!player) { router.replace('/hexaco-pvp'); return; }
    const { channel } = useHexacoPvpStore.getState();
  ...
  useEffect(() => {
    const buffer = () => {
      try { useHexacoPvpStore.getState().bufferInterruptedSnapshot(); } catch {}
    };
  ...
  useEffect(() => {
    const onVis = () => {
      const st = useHexacoPvpStore.getState();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!data || data.status !== 'playing') {
        ...
        useHexacoPvpStore.getState().reset();
        router.replace('/hexaco-pvp');
      }
    })();
  ...
  // dest：離開後去哪。默認回主頁；終局結算頁的組員「返回大廳」傳 '/hexaco-pvp'。
  async function handleAbandonRoom(dest: string = '/') {
    const { isHost, gameState: gs, room, sendMessage: send } = useHexacoPvpStore.getState();
    ...
    if (isHost) {
      ...
      if (gameInProgress) {
        ...
        try { useHexacoPvpStore.getState().persistInterruptedGame(); } catch {}
      }
  ...
  function dispatchAction(action: PvpAction) {
    if (isHost) {
      useHexacoPvpStore.getState().handlePlayerAction(myId!, action);
    } else {
  ...
  //   組員：只有「返回大廳」一個鈕 → handleAbandonRoom('/hexaco-pvp')（leaveRoom 讓出
  ...
  if (gameState.winner) {
    return (
      <GameOverModal
        ...
        onPlayAgain={isHost ? () => router.replace(`/hexaco-pvp/room/${code}`) : undefined}
        onBackToLobby={() => handleAbandonRoom(isHost ? '/' : '/hexaco-pvp')}
  ...
  const targets = mePlayer ? getTargetCounts(mePlayer.hexacoScores) : null;
        ...
        <div className="mt-2 flex flex-1 flex-col space-y-2 p-1 sm:mt-3 sm:space-y-3 sm:p-1.5">
            ...
            info={
              <>
                ...
                {isMyTurn && isDiscarding && !viewMode && !pongIntent && discardPickId === null ? (
                  ...
                  <span className="ml-auto shrink-0 font-medium">{t.doneLabel} {mePlayer.declaredSets.length}/6</span>
                )}
              </>
            }
            targets={targets ?? {}}
          ...
          {canClaim && gameState.pendingDiscard && (
            <div className="psy-panel space-y-3 rounded-[1.35rem] border p-4">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="w-24 rounded-xl ring-1 ring-[rgba(200,155,93,0.35)]">
                    <HexacoCard
                      ...
                      imageSrc={isPersonalityCard(gameState.pendingDiscard) ? `/cards/hexaco/${gameState.pendingDiscard.imageId ?? gameState.pendingDiscard.id}.webp` : undefined}
                      locale={locale}
                      fluid
                    />
                  </div>
              ...
              {/* 六维目标行（对齐单机 PongPanel）：移动端 3+3 两行，≥sm 单行 6 列。 */}
              {targets && (
          ...
          >
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {DIMENSIONS.map((d) => {
                  const score = mePlayer.hexacoScores[d];
