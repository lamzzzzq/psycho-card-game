...
// HEXACO 聯機大廳（src/app/pvp/page.tsx 的六維物理隔離副本）。差異：
//   分數源 = HEXACO-60 測評（useHexacoStore）；建房 settings.deck 固定 'hexaco'；
//   不提供大五那套「手動填分」（HEXACO 未拍板要不要，先只給去測評入口；
//   未測評照樣可玩——引擎用隨機六維分兜底，同大五規則）。
...
import { RevealDifficulty } from '@/types/hexaco-game';
...
export default function PvpLobbyPage() {
  ...
  const { player, setPlayer } = useHexacoPlayerStore();
  const hexacoScores = useHexacoStore((st) => st.scores);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (active.status === 'waiting') {
        router.replace(`/hexaco-pvp/room/${active.code}`);
        return;
      }
  ...
  function resumeActiveRoom() {
    ...
    router.replace(activeRoom.status === 'playing' ? `/hexaco-pvp/game/${activeRoom.code}` : `/hexaco-pvp/room/${activeRoom.code}`);
  }
  ...
  async function ensurePlayer() {
    ...
    const prevId = useHexacoPlayerStore.getState().player?.id;
    if (prevId && prevId !== sid) {
      try { await leaveAllRooms(prevId); } catch {}
      useHexacoPvpStore.getState().reset();
    }
    const info: PlayerInfo = { id: sid, studentId: sid, hexaco: hexacoScores, avatar };
  ...
  async function handleCreate() {
    ...
    try {
      ...
      useHexacoPvpStore.getState().reset();
      const room = await createRoom(info.id, { maxPlayers, totalRounds, deck: 'hexaco', difficulty }, info.avatar);
      router.push(`/hexaco-pvp/room/${room.code}`);
    } catch (e: any) {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      router.push(`/hexaco-pvp/room/${joinCode}`);
    } catch (e: any) {
      ...
      >
        <div className="space-y-3">
          ...
          <h1 className="psy-serif whitespace-nowrap text-[1.55rem] leading-tight text-[var(--psy-ink)] sm:whitespace-normal sm:text-6xl sm:leading-none">
            {t.title}
            <span className="ml-2 align-baseline text-base font-semibold text-[var(--psy-accent)] sm:ml-3 sm:text-3xl">· HEXACO</span>
          </h1>
        ...
        <section className="psy-panel psy-etched space-y-4 rounded-[1.6rem] p-6">
          ...
          {hexacoScores ? (
            ...
            <div className="space-y-3">
              ...
              {/* HEXACO 不做大五那套手動填分（未拍板），只給去測評入口。
                  未測評也能開/進房——引擎用隨機六維分兜底（同大五規則）。 */}
              <button onClick={() => router.push('/hexaco/assess')} className="psy-btn psy-btn-accent px-3 py-2 text-xs">
                {t.fullAssessPrefix}{HEXACO_QUESTIONS.length}{t.fullAssessSuffix}
              </button>
            </div>
          )}
        </section>
