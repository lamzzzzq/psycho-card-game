...
// HEXACO 規則頁（src/app/rules/page.tsx 的六維物理隔離副本）：文案取 HEXACO_RULES_T，
// 目標圖例六維、示例牌字母用 X（外向性）。版式/列印樣式逐行沿用。

import { useRouter } from 'next/navigation';
import { QRCodeSVG } from 'qrcode.react';
import { useLocaleStore } from '@/lib/i18n';
import { useHydrated } from '@/stores/useHydration';
import { HEXACO_RULES_T, type RuleBlock } from '@/lib/i18n/hexaco-rules';
        ...
        <>
          <div className="fig">
            <span className="fig-grp">
              <span className="fig-card">X</span>
          ...
          <div className="fig">
            <span className="fig-grp">
              <span style={{ display: 'inline-flex', gap: 3 }}>
                <span className="fig-card">X</span><span className="fig-card">X</span>
              </span>
            ...
            <span className="fig-op">=</span>
            <span className="fig-grp">
              <span className="fig-set">X X X <span className="fig-num">3</span></span>
  ...
  return (
    <div className="rules-screen">
      ...
      <div className="rules-toolbar no-print">
        <button className="rules-btn" onClick={() => router.push('/hexaco-tutorial')}>{s.backToTutorial}</button>
