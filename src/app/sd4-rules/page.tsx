...
// SD4 規則頁（src/app/hexaco-rules/page.tsx 的四維物理隔離副本）：文案取 SD4_RULES_T，
// 目標圖例四維、示例牌字母用 N（自戀）。版式/列印樣式逐行沿用。

import { useRouter } from 'next/navigation';
import { QRCodeSVG } from 'qrcode.react';
import { useLocaleStore } from '@/lib/i18n';
import { useHydrated } from '@/stores/useHydration';
import { SD4_RULES_T, type RuleBlock } from '@/lib/i18n/sd4-rules';
  ...
  const renderFig = (fig: string | undefined) => {
    if (fig === 'goal') {
      return (
        <>
          <div className="dims">
            <div className="dim"><b>M</b> {s.dimM}</div>
            <div className="dim"><b>N</b> {s.dimN}</div>
            <div className="dim"><b>P</b> {s.dimP}</div>
            <div className="dim"><b>S</b> {s.dimS}</div>
          </div>
          <div className="fig">
            {([['M', 3], ['N', 4], ['P', 2], ['S', 3]] as const).map(([d, n]) => (
        ...
        <>
          <div className="fig">
            <span className="fig-grp">
              <span className="fig-card">N</span>
          ...
          <div className="fig">
            <span className="fig-grp">
              <span style={{ display: 'inline-flex', gap: 3 }}>
                <span className="fig-card">N</span><span className="fig-card">N</span>
              </span>
            ...
            <span className="fig-op">=</span>
            <span className="fig-grp">
              <span className="fig-set">N N N <span className="fig-num">3</span></span>
  ...
  return (
    <div className="rules-screen">
      ...
      <div className="rules-toolbar no-print">
        <button className="rules-btn" onClick={() => router.push('/sd4-tutorial')}>{s.backToTutorial}</button>
