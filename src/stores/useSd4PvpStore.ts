...
// SD4 PVP store（src/stores/usePvpStore.ts 的四維物理隔離副本）。差異：sd4-* 依賴、
// 廣播頻道前綴 sdpvp-、persist key psycho-card-sd4-pvp、分數字段 sd4。
...
import { Room, RoomPlayer, RoomSettings, RealtimeMessage, PvpAction, SerializedGameState } from '@/types/sd4-pvp';
import { getRoomPlayers, updateRoomStatus, kickPlayer, dissolveRoom, leaveRoom } from '@/lib/sd4-game/room-api';
import {
  saveGameSession,
  bufferPendingSave,
  removePendingInterrupted,
  SaveGameSessionInput,
} from '@/lib/sd4-game/game-record';
import { Sd4Scores } from '@/types/sd4-game';
import { serializeGameState } from '@/lib/sd4-game/pvp-serializer';
import { initializePvpGame, applyPvpAction } from '@/lib/sd4-game/pvp-game-logic';
...
function armClaimTimer(getStore: () => Sd4PvpStore) {
  clearClaimTimer();
...
export const useSd4PvpStore = create<Sd4PvpStore>()(
  persist(
    (set, get) => ({
  room: null,
  players: [],
  ...
  subscribeRoom: (roomCode, myPlayerId) => {
    ...
    const channel = supabase.channel(`sdpvp-${roomCode}`, {
          ...
          case 'player-joined': {
            set(s => {
              ...
              if (idx >= 0) {
                ...
                // 这样后到的「重广播」能把先前建成 null 的 sd4/学号/头像补上，
                // 修复各客户端「測評狀態/学号」分歧；且不会用 null 覆盖已有的好值。
                const ex = s.players[idx];
                const merged = {
                  ...ex,
                  student_id: ex.student_id ?? payload.player.studentId,
                  sd4: ex.sd4 ?? payload.player.sd4,
                  avatar: ex.avatar ?? payload.player.avatar,
                };
                if (merged.student_id === ex.student_id && merged.sd4 === ex.sd4 && merged.avatar === ex.avatar) return s;
            ...
            if (isHost) {
              ...
              for (const p of existing) {
                get().sendMessage({
                  type: 'player-joined',
                  player: { id: p.player_id, studentId: p.student_id ?? p.player_id, sd4: p.sd4 ?? null, avatar: p.avatar },
          ...
          case 'sd4-updated':
            set(s => ({
              players: s.players.map(p =>
                p.player_id === payload.playerId ? { ...p, sd4: payload.sd4 } : p
              ),
            }));
            break;
  ...
  startGame: () => {
    ...
    // Gather SD4 scores for each player
    const sd4Map = Object.fromEntries(
      players.map(p => [p.player_id, p.sd4 ?? null]).filter(([, v]) => v !== null)
    );
    ...
    const orderedPlayers = [...players]
      ...
      .slice(0, maxPlayers);
    const rawState = initializePvpGame(orderedPlayers, sd4Map, room.settings);
  ...
  handlePlayerAction: (_fromPlayerId, action) => {
    ...
    if (newState.winner && !wasAlreadyWinner) {
      ...
      if (room) {
        // New schema: game_sessions + game_participants + sd4_snapshots.
  ...
  reset: () => {
    ...
    try { localStorage.removeItem('psycho-card-sd4-pvp'); } catch {}
  },
    }),
    {
      name: 'psycho-card-sd4-pvp',
