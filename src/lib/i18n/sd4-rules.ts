// SD4 規則頁（A4 列印版）雙語文案 —— src/lib/i18n/hexaco-rules.ts 的四維物理隔離副本。
// 正文句式逐句沿用定稿，僅按四維事實調整：6→4、HEXACO→Dark Tetrad（M/N/P/S）、
// 維度名與示例目標張數。
// 純資料模組，無 React 依賴。
...
export const SD4_RULES_T = {
  zh: {
    backToTutorial: '← 返回教學',
    printOrPdf: '🖨 列印 / 存 PDF',

    titleMain: '人格麻將',
    titleSub: 'Dark Tetrad 遊戲規則',
    scanToEnter: '掃碼進入遊戲',
    ...
    figGoalCap: '圈內數字＝目標張數 → 4 維全部歸檔即食胡並獲勝',
    figFlowDraw: '① 摸 1 張',
    ...
    sections: [
      {
        title: '🎯 一、終極目標：4維度「公開歸檔」！',
        fig: 'goal',
        blocks: [
          { t: 'p', text: '你的目標是將 Dark Tetrad 的 4 個維度（馬基維利主義、自戀、病態人格、虐待）湊齊並「公開歸檔」，最快完成的人獲勝！' },
          { t: 'p', text: '特別注意：每個維度的目標張數取決於你的測評分數。例如：你的目標可能是 M 需 3 張、N 需 4 張、P 需 2 張、S 需 3 張等。' },
          { t: 'tip', text: '初始手牌公式：你的初始手牌張數 = 4 個維度目標張數之和 − 1（少的那 1 張，要靠「碰」或「食胡」來補齊）。' },
        ],
      },
      {
        title: '🃏 二、認識卡牌',
  ...
  en: {
    ...
    titleSub: 'Dark Tetrad Game Rules',
    scanToEnter: 'Scan to enter',

    dimM: 'Machiavellianism',
    dimN: 'Narcissism',
    dimP: 'Psychopathy',
    dimS: 'Sadism',

    figGoalCap: 'The number in each chip = target count → file all 4 dimensions to Win',
    figFlowDraw: '① Draw 1',
    ...
    sections: [
      {
        title: '🎯 1. Ultimate Goal: Publicly File All 4 Dimensions!',
        fig: 'goal',
        blocks: [
          { t: 'p', text: 'Your goal is to collect and "publicly file" all 4 Dark Tetrad dimensions (Machiavellianism, Narcissism, Psychopathy, Sadism). The first to finish wins!' },
          { t: 'p', text: 'Note: the target count for each dimension depends on your assessment score. Example: your targets might be M needs 3, N needs 4, P needs 2, S needs 3, and so on.' },
          { t: 'tip', text: 'Starting-hand formula: your starting hand size = the sum of the 4 dimension targets − 1. That 1 missing card must be completed later through a "Self-draw Pong", "Intercept Pong", or your final "Win"!' },
        ],
      },
      {
      ...
      },
      {
        ...
        blocks: [
          ...
          { t: 'p', text: 'When you have filed the target counts for all 4 dimensions:' },
