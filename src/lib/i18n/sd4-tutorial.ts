// SD4 教學頁文案詞典（src/lib/i18n/hexaco-tutorial.ts 的四維物理隔離副本；句式沿用定稿，
// 僅按四維事實調整：6→4、HEXACO→Dark Tetrad、沙盒聚焦維度 情緒性→自戀、示例維度換 SD4）。
// 教學頁（tutorial）文案詞典。zh / en 同結構。
// 含 ${...} 變量的句子一律用箭頭函數（zh/en 同簽名、同參數）。
// 維度名本身已是雙語數據（DIMENSION_META.name / .nameEn），調用方按 locale 取好字符串再傳進來。

export const SD4_TUTORIAL_T = {
  zh: {
    // ── 頁眉 / 頁腳 ──
    eyebrow: 'Tutorial',
    title: '人格麻將 · Dark Tetrad · 教學',
    titleMain: '人格麻將',
    titleSub: 'Dark Tetrad 教學',
    ...
    dgIncoming: '抽到／別人棄的',
    dgLocked: '鎖定一組',
    dgGoalCaption: '圈內數字＝目標張數 → 4 維全部歸檔即食胡並獲勝',
    ...
    pvpFlow: [
      ...
      {
        title: '查看人格畫像',
        body: '答完題進入「你的人格畫像」：四維雷達圖 + 各維度得分。這一頁底部就是「聯機對戰 / 單機對戰 / 重新測評」的入口。',
        note: '測評分數決定各維度目標張數，也決定你的起始手牌數（＝四維目標張數之和 − 1）。因此每個人的開局規模與胡牌路線都不同。',
      },
      {
        title: '進入聯機對戰',
    ...
    steps: [
      {
        title: '🎯 終極目標：4維度「公開歸檔」！',
        body: '你的目標是將 Dark Tetrad 的 4 個維度（馬基維利主義、自戀、病態人格、虐待）湊齊並「公開歸檔」，最快完成的人獲勝！\n特別注意：每個維度的目標張數取決於你的測評分數。例如：你的目標可能是 M 需 3 張、N 需 4 張、P 需 2 張、S 需 3 張等。',
        hint: '初始手牌公式：你的初始手牌張數 = 4 個維度目標張數之和 − 1（少的那 1 張，要靠「碰」或「食胡」來補齊）。',
      },
      {
        title: '🃏 認識卡牌',
    ...
    // ── 沙盒：食胡演示框 ──
    huDemoBox: '食胡只在 4 個維度都完成時按。判定不成立會公開整副手牌並罰停，所以它是確認勝利，不是試探按鈕。',
    ...
    captionAfterDraw: '自戀的「目標張數」是 4。你手裏正好有 4 張自戀牌。點高亮的「自摸碰」開始選維度及點牌。',
    ...
    captionHuWindow: (name: string) =>
      `你已歸檔 3 維，只差「${name}」。對手打出的這張判定為「${name}」，正是你缺的最後一維——點高亮的「食胡」宣告勝利。`,
    captionHuSuccess: '食胡成功！四個維度全部集齊，你贏了。',
    captionDiscardPicking: '點擊要棄的牌。',
    captionDone: '你的回合結束。下一步模擬當別人棄牌時，你如何「碰 / 食胡」。',

    // ── 沙盒：feedback（reducer 內）──
    fbDraw: '抽到一張牌。現在選擇很多，先演示「查看 2 張牌」。',
    fbViewTwo: '本回合查看了 2 張牌：一張虐待，一張自戀。隱藏模式看過的牌下一輪會重置、不保留；真實牌局裏只會揭開你選的 2 張。',
    ...
    fbOpenPong: '自摸碰要先選定一個人格維度。提示：手牌裏有 4 張「自戀」，選它。',
    ...
    fbShowHu: '食胡用於宣佈勝利：當 4 個維度全部完成時按下。誤按會公開整副手牌並罰停。',
    fbWrongDimHint: (name: string) => `這張不是「${name}」，混維度會碰失敗——只選「${name}」。`,
    fbEnterHu: (name: string) => `食胡課：你已歸檔 3 維，只差「${name}」最後一組。`,
    fbHuSuccess: '食胡成功！四維集齊，宣告勝利。',

    // ── 沙盒：食胡窗口 + 缺口標籤 ──
    huWho: '對手打出一張人格牌',
    huBody: (name: string) => `判定它屬於「${name}」——正是你缺的最後一維。手裏 3 張「${name}」+ 這張 = 第 4 組，符合食胡！`,
  ...
  en: {
    // ── Header / footer ──
    eyebrow: 'Tutorial',
    title: 'Personalities Mahjong · Dark Tetrad · Tutorial',
    titleMain: 'Personalities Mahjong',
    titleSub: 'Dark Tetrad Tutorial',
    ...
    dgLocked: 'Locked set',
    dgGoalCaption: 'The number in the chip = target count → file all 4 dimensions to Win.',
    ...
    dgDiscardPile: 'DISC',
    dgWinBtn: 'Win',
    dgWinCaption: 'All 4 complete → press "Win"',
    ...
    pvpFlow: [
      ...
      },
      {
        ...
        body: 'After finishing the questions, you will see "Your Personality Portrait": a radar chart of your four dimensions along with your specific scores. The entry points for "Solo Match / Multiplayer Match / Retake Assessment" are located at the bottom of this page.',
        note: 'Your assessment scores determine the target card count for each dimension, which also sets your starting hand size (calculated as the sum of all four target counts minus one). Consequently, every player begins with a unique opening scale and a distinct path to a Win.',
      },
      {
    ...
    steps: [
      {
        title: '🎯 Ultimate Goal: Publicly File All 4 Dimensions!',
        body: 'Your goal is to collect and "publicly file" all 4 Dark Tetrad dimensions (Machiavellianism, Narcissism, Psychopathy, Sadism). The first to finish wins!\nNote: the target count for each dimension depends on your assessment score. Example: your targets might be M needs 3, N needs 4, P needs 2, S needs 3, and so on.',
        hint: 'Starting-hand formula: your starting hand size = the sum of the 4 dimension targets − 1. That 1 missing card must be completed later through a "Self-draw Pong", "Intercept Pong", or your final "Win"!',
      },
      {
    ...
    huDemoBox: 'Press Win only when all 4 dimensions are complete. A failed judgement reveals your whole hand and freezes you, so it confirms victory — it is not a probing button.',
    ...
    captionAfterDraw: 'Narcissism\'s target count is 4, and your hand has exactly 4 Narcissism cards. Tap the highlighted "Self-draw Pong" to start choosing the dimension and cards.',
    captionPongDimension: (name: string) =>
    ...
    captionHuWindow: (name: string) =>
      `You have filed 3 dimensions and only need "${name}" to win. The opponent\'s discard is verified as "${name}" — exactly the final dimension you need. Tap the highlighted "Win" to declare victory!`,
    captionHuSuccess: 'Winning card claimed! All 4 dimensions are complete — you win!',
    ...
    fbViewTwo: 'This turn you viewed 2 cards: one Sadism, one Narcissism. In Hidden mode viewed cards reset next round — not kept; in a real game only the 2 you pick are revealed.',
    ...
    fbOpenPong: 'A Self-draw Pong requires you to select a personality dimension first. Hint: your hand has 4 "Narcissism" cards — choose it.',
    ...
    fbShowHu: 'Win declares victory: press it when all 4 dimensions are complete. Pressing it by mistake reveals your whole hand and freezes you.',
    fbWrongDimHint: (name: string) => `This isn\'t "${name}". Mixing dimensions fails the Pong — pick only "${name}".`,
    fbEnterHu: (name: string) => `Winning Tutorial: you have filed 3 dimensions and only need one final set of "${name}".`,
    fbHuSuccess: 'You win! All 4 dimensions complete — victory declared.',

    // ── Sandbox: Win window + gap label ──
    huWho: 'An opponent discards a personality card',
    huBody: (name: string) => `It is judged as "${name}" — exactly the dimension you needed. Your 3 "${name}" in hand + this card = the 4th set. That\'s a Win!`,
