// HEXACO 教學頁文案詞典（src/lib/i18n/tutorial.ts 的六維物理隔離副本；句式沿用大五定稿，
// 僅按六維事實調整：5→6、OCEAN→HEXACO、沙盒聚焦維度 神經質→情緒性。用戶 2026-08-06 授權）。
// 教學頁（tutorial）文案詞典。zh / en 同結構。
// 含 ${...} 變量的句子一律用箭頭函數（zh/en 同簽名、同參數）。
// 維度名本身已是雙語數據（DIMENSION_META.name / .nameEn），調用方按 locale 取好字符串再傳進來。

export const HEXACO_TUTORIAL_T = {
  zh: {
    // ── 頁眉 / 頁腳 ──
    eyebrow: 'Tutorial',
    title: '人格麻將 · HEXACO · 教學',
    titleMain: '人格麻將',
    titleSub: 'HEXACO 教學',
    ...
    dgIncoming: '抽到／別人棄的',
    dgLocked: '鎖定一組',
    dgGoalCaption: '圈內數字＝目標張數 → 6 維全部歸檔即食胡並獲勝',
    ...
    pvpFlow: [
      ...
      {
        title: '查看人格畫像',
        body: '答完題進入「你的人格畫像」：六維雷達圖 + 各維度得分。這一頁底部就是「聯機對戰 / 單機對戰 / 重新測評」的入口。',
        note: '測評分數決定各維度目標張數，也決定你的起始手牌數（＝六維目標張數之和 − 1）。因此每個人的開局規模與胡牌路線都不同。',
      },
      {
        title: '進入聯機對戰',
    ...
    steps: [
      {
        title: '🎯 終極目標：6維度「公開歸檔」！',
        body: '你的目標是將 HEXACO 的 6 個維度（誠實謙遜、情緒性、外向性、宜人性、盡責性、開放性）湊齊並「公開歸檔」，最快完成的人獲勝！\n特別注意：每個維度的目標張數取決於你的測評分數。例如：你的目標可能是 H 需 3 張、E 需 4 張、X 需 2 張、A 需 5 張、C 需 4 張、O 需 3 張等。',
        hint: '初始手牌公式：你的初始手牌張數 = 6 個維度目標張數之和 − 1（少的那 1 張，要靠「碰」或「食胡」來補齊）。',
      },
      {
        title: '🃏 認識卡牌',
    ...
    // ── 沙盒：食胡演示框 ──
    huDemoBox: '食胡只在 6 個維度都完成時按。判定不成立會公開整副手牌並罰停，所以它是確認勝利，不是試探按鈕。',
    ...
    captionAfterDraw: '情緒性的「目標張數」是 4。你手裏正好有 4 張情緒性牌。點高亮的「自摸碰」開始選維度及點牌。',
    ...
    captionHuWindow: (name: string) =>
      `你已歸檔 5 維，只差「${name}」。對手打出的這張判定為「${name}」，正是你缺的最後一維——點高亮的「食胡」宣告勝利。`,
    captionHuSuccess: '食胡成功！六個維度全部集齊，你贏了。',
    ...
    fbOpenPong: '自摸碰要先選定一個人格維度。提示：手牌裏有 4 張「情緒性」，選它。',
    ...
    fbShowHu: '食胡用於宣佈勝利：當 6 個維度全部完成時按下。誤按會公開整副手牌並罰停。',
    fbWrongDimHint: (name: string) => `這張不是「${name}」，混維度會碰失敗——只選「${name}」。`,
    fbEnterHu: (name: string) => `食胡課：你已歸檔 5 維，只差「${name}」最後一組。`,
    fbHuSuccess: '食胡成功！六維集齊，宣告勝利。',

    // ── 沙盒：食胡窗口 + 缺口標籤 ──
    huWho: '對手打出一張人格牌',
    huBody: (name: string) => `判定它屬於「${name}」——正是你缺的最後一維。手裏 3 張「${name}」+ 這張 = 第 6 組，符合食胡！`,
  ...
  en: {
    // ── Header / footer ──
    eyebrow: 'Tutorial',
    title: 'Personalities Mahjong · HEXACO · Tutorial',
    titleMain: 'Personalities Mahjong',
    titleSub: 'HEXACO Tutorial',
    ...
    dgLocked: 'Locked set',
    dgGoalCaption: 'The number in the chip = target count → file all 6 dimensions to Win.',
    ...
    dgDiscardPile: 'DISC',
    dgWinBtn: 'Win',
    dgWinCaption: 'All 6 complete → press "Win"',
    ...
    pvpFlow: [
      ...
      },
      {
        ...
        body: 'After finishing the questions, you will see "Your Personality Portrait": a radar chart of your six dimensions along with your specific scores. The entry points for "Solo Match / Multiplayer Match / Retake Assessment" are located at the bottom of this page.',
        note: 'Your assessment scores determine the target card count for each dimension, which also sets your starting hand size (calculated as the sum of all six target counts minus one). Consequently, every player begins with a unique opening scale and a distinct path to a Win.',
      },
      {
    ...
    steps: [
      {
        title: '🎯 Ultimate Goal: Publicly File All 6 Dimensions!',
        body: 'Your goal is to collect and "publicly file" all 6 HEXACO dimensions (Honesty-Humility, Emotionality, eXtraversion, Agreeableness, Conscientiousness, Openness). The first to finish wins!\nNote: the target count for each dimension depends on your assessment score. Example: your targets might be H needs 3, E needs 4, X needs 2, A needs 5, C needs 4, O needs 3, and so on.',
        hint: 'Starting-hand formula: your starting hand size = the sum of the 6 dimension targets − 1. That 1 missing card must be completed later through a "Self-draw Pong", "Intercept Pong", or your final "Win"!',
      },
      {
    ...
    huDemoBox: 'Press Win only when all 6 dimensions are complete. A failed judgement reveals your whole hand and freezes you, so it confirms victory — it is not a probing button.',
    ...
    captionAfterDraw: 'Emotionality\'s target count is 4, and your hand has exactly 4 Emotionality cards. Tap the highlighted "Self-draw Pong" to start choosing the dimension and cards.',
    captionPongDimension: (name: string) =>
    ...
    captionHuWindow: (name: string) =>
      `You have filed 5 dimensions and only need "${name}" to win. The opponent\'s discard is verified as "${name}" — exactly the final dimension you need. Tap the highlighted "Win" to declare victory!`,
    captionHuSuccess: 'Winning card claimed! All 6 dimensions are complete — you win!',
    ...
    fbOpenPong: 'A Self-draw Pong requires you to select a personality dimension first. Hint: your hand has 4 "Emotionality" cards — choose it.',
    ...
    fbShowHu: 'Win declares victory: press it when all 6 dimensions are complete. Pressing it by mistake reveals your whole hand and freezes you.',
    fbWrongDimHint: (name: string) => `This isn\'t "${name}". Mixing dimensions fails the Pong — pick only "${name}".`,
    fbEnterHu: (name: string) => `Winning Tutorial: you have filed 5 dimensions and only need one final set of "${name}".`,
    fbHuSuccess: 'You win! All 6 dimensions complete — victory declared.',

    // ── Sandbox: Win window + gap label ──
    huWho: 'An opponent discards a personality card',
    huBody: (name: string) => `It is judged as "${name}" — exactly the dimension you needed. Your 3 "${name}" in hand + this card = the 6th set. That\'s a Win!`,
