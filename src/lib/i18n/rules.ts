...
export const RULES_T = {
    ...
    sections: [
      {
        ...
        fig: 'goal',
        blocks: [
          { t: 'p', text: '你的目標是將五大人格的 5 個維度（OCEAN）全部湊齊並「公開歸檔」，最快完成的人獲勝！' },
  ...
  en: {
    ...
    // figure captions
    figGoalCap: 'The number in each chip = how many cards that dimension needs (from your score, different per dimension) → file all five to Win',
    figFlowDraw: '① Draw 1',
    ...
    sections: [
      {
        ...
        fig: 'goal',
        blocks: [
          { t: 'p', text: 'Your goal is to collect and "publicly file" all 5 Big Five personality dimensions (OCEAN). The first player to complete their profile wins!' },
          { t: 'p', text: 'Note: The number of cards required for each dimension depends entirely on your initial psychological assessment score. This means every player\'s "path to victory (target counts)" is completely unique!' },
          { t: 'p', text: 'Example: your targets might be O needs 3, C needs 4, E needs 2, A needs 5, N needs 4.' },
          { t: 'tip', text: 'Starting-hand formula: your starting hand size = the sum of the 5 dimension targets − 1. That 1 missing card must be completed later through a "Self-draw Pong", "Intercept Pong", or your final "Win"!' },
        ],
      },
      {
      ...
      },
      {
        ...
        blocks: [
          { t: 'sub', text: '❶ Draw' },
          { t: 'p', text: 'On your turn, first draw 1 card from the deck. Now you can view your hand, choose to file your cards, or get ready to discard.' },
          { t: 'sub', text: '🔍 Core mechanic: how many cards in your hand can you see?' },
          { t: 'p', text: 'It depends on the host\'s chosen "Reveal Difficulty":' },
          { t: 'li', text: 'Open (Default): fully public! Your hand and all discarded cards show their dimension tags directly — no viewing required.' },
          { t: 'li', text: 'Semi-Open: view up to 4 cards in your hand each turn (once viewed, they remain permanently revealed); discards do not show tags.' },
          { t: 'li', text: 'Hidden: view only 2 cards each turn, which automatically reset (turn back over) after a round; discards do not show tags.' },
          { t: 'sub', text: '❷ Discard' },
          { t: 'p', text: 'Pick 1 unwanted card from your hand and play it into the central "discard". This discard opens a brief, few-second "psychological read window", during which other players can decide whether to seize it for an "Intercept Pong" or a final "Win".' },
        ],
      },
      {
        ...
        blocks: [
          ...
          { t: 'p', text: 'When a dimension\'s cards reach your target count, you can declare "Pong" to lock (file) that dimension.' },
          { t: 'li', text: 'Self-draw Pong: on your own turn, use the cards from your "hand + the just-drawn card" to select the matching-dimension cards that meet your target count, then file them. (Limited to once per turn)' },
          { t: 'li', text: 'Intercept Pong (grab): within another player\'s "read window", if your card count for a specific dimension is "just 1 short" of its target, you can instantly seize that discard to complete your file! (First-come, first-served)' },
          { t: 'warn', text: '⚠️ The system will not tell you which dimension is ready! All 5 dimension buttons will light up; you must count the cards and verify the dimensions yourself. Select or place them incorrectly, and you will be "Frozen"!' },
          { t: 'sub', text: '🏆 Win (declare victory)' },
          { t: 'p', text: 'When your filed dimensions + your current hand + the current judged card perfectly complete the target counts for all 5 dimensions:' },
          { t: 'li', text: 'Self-draw Win: you draw the final key card on your own turn.' },
          { t: 'li', text: 'Intercept Win: an opponent discards the exact final card you need — intercept it directly to claim victory!' },
          { t: 'warn', text: '⚠️ Declaring a Win without counting correctly? A failed judgment carries a harsh "Frozen" penalty!' },
        ],
      },
      {
        title: '🚫 5. Penalty: Frozen',
        blocks: [
          { t: 'p', text: 'If you fail a "Self-draw Pong", "Intercept Pong", or a "Win" declaration, you will receive the following penalties:' },
          { t: 'li', text: 'Frozen for one turn: your next turn is automatically skipped (no drawing, no discarding).' },
          { t: 'li', text: 'Forced reveal: the cards involved in the failed Pong (or your "entire hand" if you failed a Win) must be immediately revealed to all players.' },
          { t: 'li', text: 'Locked out: while frozen, you cannot participate in anyone else\'s "read window" (meaning you cannot Intercept Pong or declare a Win).' },
          { t: 'li', text: 'Public shame badge: a massive "⛔ Frozen" indicator will be slapped onto your avatar.' },
