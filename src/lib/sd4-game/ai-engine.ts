// SD4 AI 引擎（src/lib/ai-engine.ts 的四維物理隔離副本，經 hexaco-game 版移植）。
// 決策邏輯逐行一致；hard AI 的「對手強維」仍取每對手棄得最少的前 2 維（2-of-4）。
import {
  GameCard,
  Player,
  GameAction,
...
export function makeAIHuDecision(
  ...
  const handByDim: Record<Dimension, number> = { M: 0, N: 0, P: 0, S: 0 };
  ...
  for (const action of actionLog) {
    if (action.type === 'discard' && action.card && action.playerId !== player.id) {
      if (!opponentDiscards[action.playerId]) {
        opponentDiscards[action.playerId] = { M: 0, N: 0, P: 0, S: 0 };
      }
