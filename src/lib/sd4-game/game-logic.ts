// SD4 遊戲引擎（src/lib/game-logic.ts 的四維物理隔離副本，經 hexaco-game 版移植）。
// 規則邏輯與 Big Five 版逐行一致（食胡/碰/自摸碰/罰停/離場全套），僅：
//   1. 類型換成 @/types/sd4-game（四維 M/N/P/S）
//   2. bigFiveScores → sd4Scores
//   3. attemptHu 的 handByDim 字面量為四鍵（M/N/P/S）
...
function createPlayer(
  ...
  return {
    id, name, nameEn, avatar, hand, isHuman, sd4Scores,
    declaredSets: [],
    skipNextTurn: false,
    revealedHand: false,
  };
...
export function attemptHu(state: GameState, playerIndex: number): GameState {
  ...
  const handByDim: Record<Dimension, PersonalityCard[]> = { M: [], N: [], P: [], S: [] };
...
export function pongCard(
  ...
  const targets = getTargetCounts(ponger.sd4Scores);
