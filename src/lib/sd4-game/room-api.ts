// SD4 PVP 房間 API（src/lib/room-api.ts 的物理隔離副本）：rooms/room_players 基礎設施
// 共用同一套表（房號全局唯一），差異僅 ① 分數列讀寫 players.sd4（不碰 big_five）
// ② joinRoom 校驗 settings.deck === 'sd4'，防止從 SD4 大廳加進大五/HEXACO 房。
import { supabase } from '@/lib/supabase';
import { Room, RoomSettings, RoomPlayer, PlayerInfo, RoomStatus } from '@/types/sd4-pvp';
...
export async function upsertPlayer(player: PlayerInfo) {
  ...
  if (error.code === '23505') {
    ...
    const { error: updateError } = await supabase
      .from('players')
      .update({ student_id: player.studentId, sd4: player.sd4 })
      .eq('id', player.id);
...
export async function joinRoom(roomCode: string, playerId: string, avatar?: string): Promise<{ room: Room; seatIndex: number }> {
  ...
  // 牌組守衛：這裏只收 SD4（Dark Tetrad）房。輸了別的牌組的房號 → 明確告知去對應聯機大廳加入。
