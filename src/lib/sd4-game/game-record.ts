/**
 * Persist a finished SD4 game to Supabase（src/lib/game-record.ts 的物理隔離副本，
 * 落 sd4_snapshots / sd4_game_sessions / sd4_game_participants 平行三表，
 * localStorage 緩衝鍵也獨立——與大五互不串行）。
 *
 * Schema lives in supabase/migrations/0028_sd4_pvp.sql.
 * Three tables:
 *   - sd4_snapshots: 學號 ↔ 當次比賽使用的人格分數對應（不覆蓋歷史）
...
import { GameState, Sd4Scores, Player, PlayerId } from '@/types/sd4-game';
import { getPlayerScore, getRankings } from '@/lib/sd4-game/game-logic';
...
/** Insert one SD4 snapshot. Returns inserted row id. */
async function insertSnapshot(input: {
  ...
  // 客户端生成 id：sd4_snapshots 开了 INSERT-only RLS（隐私，不给 SELECT），
...
const PENDING_KEY = 'psycho-card-sd4-pending-saves';
...
const PVP_STORE_KEY = 'psycho-card-sd4-pvp';
...
async function saveInner(input: SaveGameSessionInput): Promise<string | null> {
  try {
    ...
    // 2) Snapshot SD4 scores for every non-AI seat (one snapshot per participant).
    ...
    const rows = input.finalState.players.map((p, idx) => {
      ...
      return {
        ...
        // 分数只存进 sd4_snapshots（已锁 SELECT），这里仅留 FK，不再反规范化分数列，
        // 避免 anon 经公开的 /stats 读到「学号→分数」。见 identity-plan 隐私残留 todo。
        sd4_snapshot_id: meta?.playerId ? snapshotByPlayerId.get(meta.playerId) ?? null : null,
    ...
    const { error: pErr } = await supabase.from('sd4_game_participants').insert(rows);
    if (pErr) {
