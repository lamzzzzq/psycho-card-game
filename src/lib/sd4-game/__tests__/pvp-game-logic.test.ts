...
import type { RoomPlayer, RoomSettings, PvpAction } from '@/types/sd4-pvp';
import type { Sd4Scores } from '@/types/sd4-game';
import { makeSd4, makeCard, makeGameState, makePlayer, resetCardIds } from '@/test/sd4-fixtures';
...
describe('initializePvpGame', () => {
  it('builds a game state with the supplied player count', () => {
    ...
    const sd4Map: Record<string, Sd4Scores> = {
      'uuid-a': makeSd4({ M: 4 }),
      'uuid-b': makeSd4({ N: 5 }),
      'uuid-c': makeSd4(),
    };
    const state = initializePvpGame(players, sd4Map, settings);
  ...
  it('falls back to random Big Five when player has no scores in the map', () => {
    ...
    const scores = state.players[0].sd4Scores;
    expect(scores.M).toBeGreaterThanOrEqual(1);
    expect(scores.M).toBeLessThanOrEqual(5);
  });

  it('respects supplied Big Five for known players', () => {
    const myScores = makeSd4({ M: 4.7, S: 1.3 });
    const players = makeRoomPlayers(['uuid-a']);
    const state = initializePvpGame(players, { 'uuid-a': myScores }, settings);
    expect(state.players[0].sd4Scores).toEqual(myScores);
  });
...
describe('applyPvpAction — security guards', () => {
  ...
  it('rejects pong outside claim-window', () => {
    const state = stateWithCustomPlayers();
    state.phase = 'drawing';
    const action: PvpAction = { type: 'pong', dimension: 'M', handCardIds: [] };
    const result = applyPvpAction(state, 'uuid-b', action);
    expect(result).toBe(state);
  });
  ...
  it('maps seats by engine state, not room roster: actions stay correct after a seat is removed from the roster', () => {
    ...
    state.pendingDiscard = makeCard('M');
