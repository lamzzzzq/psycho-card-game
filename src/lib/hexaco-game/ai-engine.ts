// HEXACO AI 引擎（src/lib/ai-engine.ts 的六維物理隔離副本，2026-08-05 複製）。
// 決策邏輯逐行一致；hard AI 的「對手強維」仍取每對手棄得最少的前 2 維（2-of-6）。
import {
  GameCard,
  Player,
  GameAction,
...
export function makeAIHuDecision(
  ...
  const handByDim: Record<Dimension, number> = { H: 0, E: 0, X: 0, A: 0, C: 0, O: 0 };
  ...
  for (const action of actionLog) {
    if (action.type === 'discard' && action.card && action.playerId !== player.id) {
      if (!opponentDiscards[action.playerId]) {
        opponentDiscards[action.playerId] = { H: 0, E: 0, X: 0, A: 0, C: 0, O: 0 };
      }
