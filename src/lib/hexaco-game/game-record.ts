/**
 * Persist a finished HEXACO game to Supabase（src/lib/game-record.ts 的物理隔離副本，
 * 落 hexaco_snapshots / hexaco_game_sessions / hexaco_game_participants 平行三表，
 * localStorage 緩衝鍵也獨立——與大五互不串行）。
 *
 * Schema lives in supabase/migrations/0027_hexaco_pvp.sql.
 * Three tables:
 *   - hexaco_snapshots: 學號 ↔ 當次比賽使用的人格分數對應（不覆蓋歷史）
...
import { GameState, HexacoScores, Player, PlayerId } from '@/types/hexaco-game';
import { getPlayerScore, getRankings } from '@/lib/hexaco-game/game-logic';
...
/** Insert one HEXACO snapshot. Returns inserted row id. */
async function insertSnapshot(input: {
  ...
  // 客户端生成 id：hexaco_snapshots 开了 INSERT-only RLS（隐私，不给 SELECT），
  // 所以不能用 .insert().select('id') 回读——那样 RETURNING 会被 RLS 拦住整条回滚。
  const id =
    typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : undefined;
  const { error } = await supabase
    .from('hexaco_snapshots')
    .insert({
      ...(id ? { id } : {}),
...
const PENDING_KEY = 'psycho-card-hexaco-pending-saves';
...
const PVP_STORE_KEY = 'psycho-card-hexaco-pvp';
...
async function saveInner(input: SaveGameSessionInput): Promise<string | null> {
  try {
    ...
    // 2) Snapshot HEXACO scores for every non-AI seat (one snapshot per participant).
    ...
    for (const meta of input.seatMeta) {
      ...
      const snapshotId = await insertSnapshot({
        ...
        scores: seatPlayer.hexacoScores,
        source: 'game-start',
      });
    ...
    const rows = input.finalState.players.map((p, idx) => {
      ...
      return {
        ...
        // 分数只存进 hexaco_snapshots（已锁 SELECT），这里仅留 FK，不再反规范化分数列，
        // 避免 anon 经公开的 /stats 读到「学号→分数」。见 identity-plan 隐私残留 todo。
        hexaco_snapshot_id: meta?.playerId ? snapshotByPlayerId.get(meta.playerId) ?? null : null,
    ...
    const { error: pErr } = await supabase.from('hexaco_game_participants').insert(rows);
    if (pErr) {
