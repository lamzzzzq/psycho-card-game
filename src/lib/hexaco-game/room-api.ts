// HEXACO PVP 房間 API（src/lib/room-api.ts 的物理隔離副本）：rooms/room_players 基礎設施
// 共用同一套表（房號全局唯一），差異僅 ① 分數列讀寫 players.hexaco（不碰 big_five）
// ② joinRoom 校驗 settings.deck === 'hexaco'，防止從 HEXACO 大廳加進大五房。
import { supabase } from '@/lib/supabase';
import { Room, RoomSettings, RoomPlayer, PlayerInfo, RoomStatus } from '@/types/hexaco-pvp';
...
export async function upsertPlayer(player: PlayerInfo) {
  ...
  if (error.code === '23505') {
    ...
    const { error: updateError } = await supabase
      .from('players')
      .update({ student_id: player.studentId, hexaco: player.hexaco })
      .eq('id', player.id);
