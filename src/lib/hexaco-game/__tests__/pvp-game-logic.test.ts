...
import type { RoomPlayer, RoomSettings, PvpAction } from '@/types/hexaco-pvp';
import type { HexacoScores } from '@/types/hexaco-game';
import { makeHexaco, makeCard, makeGameState, makePlayer, resetCardIds } from '@/test/hexaco-fixtures';
...
describe('initializePvpGame', () => {
  it('builds a game state with the supplied player count', () => {
    ...
    const hexacoMap: Record<string, HexacoScores> = {
      'uuid-a': makeHexaco({ O: 4 }),
      'uuid-b': makeHexaco({ C: 5 }),
      'uuid-c': makeHexaco(),
    };
    const state = initializePvpGame(players, hexacoMap, settings);
  ...
  it('falls back to random Big Five when player has no scores in the map', () => {
    ...
    const scores = state.players[0].hexacoScores;
  ...
  it('respects supplied Big Five for known players', () => {
    const myScores = makeHexaco({ O: 4.7, H: 1.3 });
    const players = makeRoomPlayers(['uuid-a']);
    const state = initializePvpGame(players, { 'uuid-a': myScores }, settings);
    expect(state.players[0].hexacoScores).toEqual(myScores);
  });
