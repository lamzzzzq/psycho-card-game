/**
 * HEXACO 牌库缩放 + 发牌安全回归（deck-scaling.test.ts 的六维物理隔离副本）。
 *
 * 平衡决策（2026-08）：
 *   - 4 人：96 人格 + 12 knowledge（108 张）
 *   - 2-3 人：72 人格 + 8 knowledge（80 张）
 *   - 手牌 = 六维 sum-1（可变）
 *
 * 推导：每维张数照抄 Big Five（12/16），knowledge 张数不变（8/12）→
 * 手牌与总牌库同时 ×1.2、K 不动 → 手上 knowledge 轮均期望与 Big Five 持平（红线 ≤1）。
...
import { DIMENSIONS, isPersonalityCard, isDummyCard } from '@/types/hexaco-game';
import type { GameState, Player, PlayerId, HexacoScores, Dimension } from '@/types/hexaco-game';

function band(lo: number, hi: number): HexacoScores {
  const r = () => Math.round((lo + Math.random() * (hi - lo)) * 10) / 10;
  return { H: r(), E: r(), X: r(), A: r(), C: r(), O: r() };
}

describe('deck composition', () => {
  it('deckConfigFor scales by player count', () => {
    expect(deckConfigFor(2)).toEqual({ personality: 72, dummy: 8 });
    expect(deckConfigFor(3)).toEqual({ personality: 72, dummy: 8 });
    expect(deckConfigFor(4)).toEqual({ personality: 96, dummy: 12 });
  });

  it('createShuffledDeck sizes: 3人=80, 4人=108', () => {
    expect(createShuffledDeck(3).length).toBe(80);
    expect(createShuffledDeck(4).length).toBe(108);
  });

  it('generatePersonalityCards(96) is balanced 16 per dimension with unique ids', () => {
    const cards = generatePersonalityCards(96);
    expect(cards.length).toBe(96);
    const byDim: Record<Dimension, number> = { H: 0, E: 0, X: 0, A: 0, C: 0, O: 0 };
    for (const c of cards) byDim[c.dimension]++;
    for (const d of DIMENSIONS) expect(byDim[d]).toBe(16);
    expect(new Set(cards.map((c) => c.id)).size).toBe(96); // 无 id 碰撞
  });

  it('generatePersonalityCards(72) is balanced 12 per dimension', () => {
    const cards = generatePersonalityCards(72);
  ...
  it('dummy ids never collide with personality ids', () => {
    const pers = generatePersonalityCards(96);
...
function runFull(
  ...
  let state: GameState = {
    ...
    players: hands.map((hand, i): Player => ({
      ...
      isHuman: false, hexacoScores: scores[i], declaredSets: [], skipNextTurn: false, revealedHand: false,
    })),
...
// HEXACO 的 K 张数不变 + 牌库/手牌同比 ×1.2，期望应与 Big Five 持平。
