/**
 * HEXACO PVP game logic wrapper（src/lib/pvp-game-logic.ts 的六維物理隔離副本）。
...
import { RoomPlayer, RoomSettings, PvpAction } from '@/types/hexaco-pvp';
import {
  drawCard,
  discardCard,
  attemptHu,
  ...
  skipPong,
  markPlayerLeft,
} from '@/lib/hexaco-game/game-logic';
import { createShuffledDeck, dealCardsVariable } from '@/lib/hexaco-game/card-engine';

// ── Initialize ───────────────────────────────────────────────────────────────

export function initializePvpGame(
  orderedPlayers: RoomPlayer[],
  hexacoMap: Record<string, HexacoScores>,
  settings: RoomSettings
): GameState {
  const allScores = orderedPlayers.map(p => hexacoMap[p.player_id] ?? randomHexaco());
...
// 未完成 HEXACO 測評的玩家用隨機分兜底（同大五 randomBigFive 的規則）。
function randomHexaco(): HexacoScores {
