...
// 六维（H-E-X-A-C-O）：名称(中/英) + 定义句(def) + 高/低分说明(hl) + 与大五对照注记(note) + 维度色。
// 文案依据 The HEXACO Model_20260723 官方文档（中英双语）。样式与 BigFiveIntro 完全一致。
const DIMS = [
  {
    ...
    hlEn: 'Higher scorers tend to avoid cheating or manipulating others and place less importance on wealth, status, and special treatment; lower scorers may be more willing to use flattery, bend rules for personal benefit, or seek status and recognition.',
    noteZh: '這是 HEXACO 模型中特有的向度，在大五人格模型中並沒有直接對應的獨立向度。',
  ...
  },
  {
    ...
    hlEn: 'Higher scorers may be more concerned about danger, form stronger emotional bonds, and seek support during difficult situations; lower scorers may be less fearful, less sentimental, and more emotionally independent.',
    noteZh: '情緒性與大五人格中的神經質有關，但兩者並不相同。在 HEXACO 模型中，憤怒和易怒主要反映於較低的宜人性，而不是較高的情緒性。',
  ...
  },
  {
    ...
    hlEn: 'Higher scorers tend to enjoy social interaction, feel confident around others, and show greater energy and enthusiasm; lower scorers may be quieter, more reserved, and more comfortable with less social stimulation.',
    noteZh: '這個向度與大五人格中的外向性大致相似。',
  ...
  },
  {
    ...
    hlEn: 'Higher scorers tend to manage their anger, forgive others, and remain flexible during disagreements; lower scorers may be more critical, stubborn, quick-tempered, or likely to hold a grudge.',
    noteZh: '這與大五人格中的宜人性有所不同——大五的宜人性較廣泛地着重同理心、信任、合作和對他人的關心。',
  ...
  },
  {
    ...
    hlEn: 'Higher scorers tend to plan ahead, work carefully, and persist with difficult tasks; lower scorers may prefer flexibility and spontaneity and may be less organised.',
    noteZh: '這個向度與大五人格中的盡責性大致相似。',
  ...
  },
  {
    ...
    hlEn: 'Higher scorers tend to enjoy learning, using their imagination, and exploring different perspectives; lower scorers may prefer familiar ideas, practical activities, and traditional approaches.',
    noteZh: '這個向度與大五人格中的開放性大致相似。',
 ...
 * 六维用双列表格呈现（名称｜定义 + 高/低分说明 + 与大五对照注记）。
 * References 依 APA：期刊名/卷号斜体；HEXACO Inventory 标题斜体（依原文档标注）。
 */
  ...
  return (
    <section className="w-full space-y-5 text-left">
      <h2 className="psy-serif text-xl text-[var(--psy-ink)] sm:text-2xl">
        {isEn ? 'The HEXACO Personality Model' : 'HEXACO 人格模型'}
      </h2>

      <p className="text-pretty text-[15px] leading-7 text-[var(--psy-ink-soft)] sm:leading-8">
        {isEn ? (
          <>
            The <strong>HEXACO model</strong> describes personality using six broad dimensions. The name{' '}
            <strong>HEXACO</strong> stands for Honesty–Humility, Emotionality, eXtraversion, Agreeableness,
            Conscientiousness, and Openness:
          </>
        ) : (
          <>
            <strong>HEXACO 人格模型</strong>以六個廣泛的人格向度來描述人格。名稱 <strong>HEXACO</strong> 代表誠實－謙遜、情緒性、外向性、宜人性、盡責性和開放性六個向度：
          </>
        )}
      </p>
      ...
      <p className="text-pretty text-[15px] leading-7 text-[var(--psy-ink-soft)] sm:leading-8">
        {isEn ? (
          <>
            In this game, you will learn more about your <strong>HEXACO personality</strong> profile. Your
          ...
          <>
            在這個遊戲中，你將進一步認識自己的 <strong>HEXACO 人格</strong>。你的測評結果將影響你在麻將中的玩法，讓你透過互動學習探索自己的人格特質，並加深對六個 HEXACO 人格向度的理解。
          </>
        )}
      </p>
