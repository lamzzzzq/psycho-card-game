/**
 * HEXACO 卡面專用的文字處理與字級模型。**只被 HexacoCard 引用，Big Five 完全不經過這裡。**
 *
 * 為什麼要另做一套：HEXACO 題面中位 65、最長 95 字元，是 Big Five（中位 25、最長 52）的 2～3 倍。
 * Big Five 那套「按字數查表」的階梯套到這麼長的句子上，會把整副牌壓成一片小字。
 */
...
export function resolveHexacoLabel(text: string, textEn: string | undefined, locale: 'zh' | 'en'): string {
...
// ── 字級 ──────────────────────────────────────────────────────────────
/**
 * 底框文字面板的可用寬高（cqw = 卡寬的 1%）。與 HexacoCard 的 roomy 幾何對應：
 * 左右各留 7.5% → 寬 85；底框高 266 單位、上下內距共 16 → (266-16)/4 = 62.5。
 */
export const HEXACO_PANEL = { width: 85, height: 62.5 };
...
export function getHexacoTextLayout(label: string, locale: 'zh' | 'en') {
  const steps = locale === 'en' ? EN_STEPS : ZH_STEPS;
  const length = Array.from(label).length;
  const maxHeight = HEXACO_PANEL.height * HEIGHT_SAFETY;
  ...
  for (const fontSize of steps) {
    ...
    if (locale === 'en') {
      const maxEm = (HEXACO_PANEL.width / fontSize) * WIDTH_SAFETY;
      if (longestWordEm(label) > maxEm) continue; // 最長單詞塞不進一行 → 降檔，免得被硬切
      lines = countEnLines(label, maxEm);
    } else {
      lines = Math.ceil((length * ZH_WRAP_SLACK) / (HEXACO_PANEL.width / fontSize));
    }
