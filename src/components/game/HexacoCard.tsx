 ...
 *
 * 2026-08-05 為 HEXACO 遊戲桌補全 OrnateCard 的三個玩法功能（幾何按 roomy 重算）：
 *   - 知識牌（isDummy + description）：極坐標暗紋 + 術語在拱區/定義在底框
 *   - 揭示維度角標（revealedDimension）：配色取 HEXACO_GAME_META
...
import type { Dimension } from '@/types/hexaco-game';
...
export function HexacoCard({
  ...
  const glowId = `hglow-${uid}`, vigId = `hvig-${uid}`;
  ...
  const label = isKnowledge
    ...
    : resolveHexacoLabel(text, textEn, locale);
  ...
  return (
    <div className="shrink-0" style={wrapperStyle}>
      ...
      >
        ...
        {/* 底框文字：人格題面（getHexacoTextLayout 按面板寬高挑檔，完整呈現不截斷）
        ...
        {revealedDimension && (() => {
          const meta = HEXACO_GAME_META[revealedDimension];
