...
import { DeclaredSet, PersonalityCard } from '@/types/sd4-game';
...
export function DeclaredArea({
  ...
  const dimName = (d: PersonalityCard['dimension']) => (locale === 'en' ? SD4_GAME_META[d].nameEn : SD4_GAME_META[d].name);
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {set.cards.map((card) => (
                    ...
                    >
                      <Sd4Card {...cardToSd4Props(card, locale)} width={56} />
                    </button>
                  ))}
                </div>
              </div>
      ...
      {detailCard && (
        <div className="grid gap-5 sm:grid-cols-[auto_1fr] sm:items-center">
          <div className="flex justify-center">
            <Sd4Card {...cardToSd4Props(detailCard, locale)} revealedDimension={detailCard.dimension} width={200} />
          </div>
          <div className="space-y-4">
            <div>
                ...
                <div className={`flex flex-wrap ${wide ? 'gap-1.5' : 'gap-2'}`}>
                  {set.cards.map((card) => (
                    <button
                      ...
                      aria-label={`${t.viewWord}: ${card.text}`}
                    >
                      <Sd4Card {...cardToSd4Props(card, locale)} width={wide ? 52 : 56} />
                    </button>
                  ))}
                </div>
              </div>
