...
import { GameCard, Dimension, DIMENSIONS, Player, isPersonalityCard } from '@/types/sd4-game';
...
import { cardToSd4Props } from './cardToSd4Props';
...
export function PongPanel({
  ...
  const targets = getTargetCounts(player.sd4Scores);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 sm:gap-6">
        <div className="flex flex-col items-center">
          <div className="rounded-xl p-1" style={{ backgroundColor: 'rgba(200, 155, 93, 0.08)', boxShadow: 'inset 0 0 0 1px rgba(200,155,93,0.18)' }}>
            <Sd4Card {...cardToSd4Props(pendingCard, locale)} width={108} revealedDimension={revealPendingDimension ? pendingDim : null} />
          </div>
        </div>
      ...
      {/* 四維目标行：全端显示（原 sm:hidden 只在移动端）。已归档维度划线花掉。 */}
      {/* 四維：單行 4 列放得下（與歸檔進度卡同一套響應規則）。 */}
