...
import { GameAction, PlayerId } from '@/types/hexaco-game';
...
export function useGameFeedback(actions: GameAction[], players: MinimalPlayer[], locale: Locale = 'zh') {
  ...
  useEffect(() => {
    ...
    const dimNameOf = (meta: typeof HEXACO_GAME_META[keyof typeof HEXACO_GAME_META] | null) =>
    ...
    if (latest.type === 'pong-success') {
      ...
      const meta = latest.dimension ? HEXACO_GAME_META[latest.dimension] : null;
      setPops((p) => [
        ...p,
        {
          id: ++popIdRef.current,
