...
import { Dimension, DIMENSIONS } from '@/types/hexaco-game';
import { HEXACO_GAME_META } from '@/data/hexaco-game-dimensions';
import type { Locale } from '@/lib/i18n';

// 回合信息 + 6 維歸檔進度卡（components/game/FilingProgressCard 的 HEXACO 物理隔離副本）。
// 與 Big Five 版唯一的結構差異：6 維同行太擠 → 移動端 3+3 兩行（grid-cols-3），
// ≥sm 恢復單行 6 列（sm:grid-cols-6）。其餘版式/字號/卡位規則逐行沿用。
//
...
export function FilingProgressCard({
  ...
  return (
    ...
    <div className="psy-panel psy-etched flex shrink-0 flex-col gap-2 rounded-[1.2rem] p-1.5 sm:p-2.5">
      ...
      {/* 第二行：6 維 —— 角標 + 卡位。
          移動端 6 格同行太擠（一格只剩 ~55px，單詞/卡位都塞不下）→ 3+3 兩行；
          ≥sm 單行 6 列。collapsible 收起時整塊不渲染（省豎向空間）。 */}
      {!(collapsible && collapsed) && (
      <div className="grid grid-cols-3 gap-1 sm:grid-cols-6 sm:gap-2.5" aria-label={en ? 'Filing progress' : '歸檔進度'}>
        {DIMENSIONS.map((dimension) => {
          const meta = HEXACO_GAME_META[dimension];
