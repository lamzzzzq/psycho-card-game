...
import { DeclaredSet } from '@/types/hexaco-game';
...
interface GameOverModalProps {
  ...
  /** 不傳 = 不渲染「再來一局」（與 Big Five 版同款 API；HEXACO 目前單機恆傳）。 */
  onPlayAgain?: () => void;
  onBackToLobby: () => void;
  locale?: Locale;
  /** 返回鍵文案。缺省「返回主頁/Home」。 */
...
export function GameOverModal({
  ...
  const hasFullWinner = (winner?.declaredSets.length ?? 0) === 6;
  ...
  return (
    ...
    >
      ...
      >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium text-[var(--psy-ink)]">
                      {declaredCount}/6 {tg.archiveCount}
                    </div>
        ...
        {/* #8 局末概念小测（可选，激励读知识卡） */}
        <KnowledgeQuiz locale={locale} model="hexaco" />
        ...
        <div className="mx-auto flex w-full max-w-lg gap-3">
          ...
          {/* 只剩一個鈕時它就是主行動：吃滿寬 + 主色（與 Big Five 版同款）。 */}
          <button
